...
APP_DIR="$ROOT/lsby-punch-in.app"
DATA_DIR="$ROOT/data"
DB_DIR="$DATA_DIR/db"
...
APP_PROCESS_NAME="lsby-punch-in"
...
expand_validated_zip() {
  ...
  local staged_app="$STAGING_DIR/lsby-punch-in.app"
  local staged_pkg="$staged_app/Contents/Resources/app/package.json"
  local staged_bin="$staged_app/Contents/MacOS/lsby-punch-in"
...
invoke_prisma() {
  ...
  local electron_bin="$macos_dir/lsby-punch-in-bin"
  if [ ! -f "$electron_bin" ]; then
    electron_bin="$macos_dir/lsby-punch-in"
  fi
...
STAGED_APP_DIR="$STAGING_DIR/lsby-punch-in.app"
...
if [ -d "$ROOT/lsby-punch-in.app" ]; then
  chmod -R +x "$ROOT/lsby-punch-in.app/Contents/MacOS" 2>/dev/null || true
  xattr -dr com.apple.quarantine "$ROOT/lsby-punch-in.app" 2>/dev/null || true
fi
